...
typedef vector<complex_num> samples;

// rearrange array
// @param: array of audio signal (all real values)
void rearrange(samples &samples){
    samples even, odd;
    int N = samples.size();
    for(int i = 0; i < N; i++){
        if(i % 2 == 0){
...
samples FFT(samples samples){
    ...
    if(N == 1){
        ...
        complex<double> W_N = polar(1.0,(-2*pi)/N);;  // root of Unity
        ...
        // conquer
        samples Y_even = FFT(sample_even);
        samples Y_odd = FFT(sample_odd);
